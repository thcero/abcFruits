...
export default function Basket() {
  ...
  // ref mirrors isExpanded so the PanResponder closure can read it without going stale
  ...
  const expand = () => {
    ...
    setIsExpanded(true);
    Animated.spring(baskHeight, { toValue: expanHeight, useNativeDriver: false }).start();
    Animated.spring(baskWidth, { toValue: expanWidth, useNativeDriver: false }).start();
  };

  const collapse = () => {
    isExpandedRef.current = false;
    setIsExpanded(false);
    Animated.spring(baskHeight, { toValue: collHeight, useNativeDriver: false }).start();
    Animated.spring(baskWidth, { toValue: collWidth, useNativeDriver: false }).start();
  };
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (_, { dy }) => {
        if (dy < -30 && !isExpandedRef.current) expand();   // swipe up → open
        ...
        {isExpanded && (items.length ? (
          items.map((fruit, index) => (
            <View key={index} style={styles.itemRow}>
              ...
              <TouchableOpacity style={styles.itemIconName} onPress={() => navigation.navigate("FruitScreen", { fruitName: fruit.name })}>
                <Image source={fruitIconImgSources[fruit.name]} style={styles.itemIcon} />
                <CustomText style={styles.itemText} numberOfLines={1}>  {fruit.name}</CustomText>
              </TouchableOpacity>
              <CustomText style={[styles.itemText, { color: theme.colors.bananaSkin }]}>{"  "}x {fruit.quantity}{"   "}</CustomText>
          ...
          <CustomText padding="large" style={{ color: "#fff", fontSize: 17.6 }}>Basket is empty</CustomText>
        ))}
      </Animated.View>
    </View>
  );
